...
func NewVacancyPostgres(db *gorm.DB, logg *logger.Logger) *VacancyPostgres {
	return &VacancyPostgres{db: db, logg: logg}
}
...
func (r *VacancyPostgres) Search(userId uint, page int64, q string) ([]DTO.VacancyDTO, models.PaginationData, error) {
	...
	var ids []string
	if err := r.db.Model(&models.Resume{}).Where("applicant_id = ?", userId).
	...
	dbBefore := r.db.Table("vacancies").
		Select("vacancies.id as ID, post, description, employer_id, "+
			"vacancies.created_at as created_at, vacancies.updated_at as updated_at, status").
...
func (r *VacancyPostgres) Update(vacancyId uint, input DTO.VacancyUpdate) error {
	...
	return r.db.Model(&vacancy).Updates(args).Error
}
...
func (r *VacancyPostgres) Delete(vacancyId uint) error {
	...
	if err := r.db.Unscoped().Delete(&models.Vacancy{}, vacancyId).Error; err != nil {
		r.logg.Error(err)
		tx.Rollback()
		return err
	}
